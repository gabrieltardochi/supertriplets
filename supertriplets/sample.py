...
class TextSample:
    ...
    def _validate_input(self):
        assert (
            type(self.text) == str
        ), f"{self.__class__.__name__} 'text' should be a string"
        assert (
            type(self.label) == int
        ), f"{self.__class__.__name__} 'label' should be an integer"
...
class ImageSample:
    ...
    def _validate_input(self):
        assert (
            type(self.image_path) == str
        ), f"{self.__class__.__name__} 'image_path' should be a string"
        assert os.path.exists(
            self.image_path
        ), f"{self.__class__.__name__} 'image_path' should exist"
        assert (
            type(self.label) == int
        ), f"{self.__class__.__name__} 'label' should be an integer"

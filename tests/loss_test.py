import torch
import pytest
from supertriplets.loss import BaseBatchTripletMiner, BatchHardTripletLoss, BatchHardSoftMarginTripletLoss
...
def test_base_batch_triplet_miner(torch_labels):
    ...
    pos_expected = torch.tensor([[False, False, False, True],
        [False, False, True, False],
        [False,  True, False, False],
        [ True, False, False, False]])
    neg_expected = torch.tensor([[False,  True,  True, False],
...
def test_batch_hard_triplet_loss(distance, expected_loss):
    embeddings = torch.tensor([[0.1, 0.3, 0.5],
                               [0.6, 0.8, 1.0],
                               [0.8, 0.6, 0.0],
                               [0.9, 0.2, -0.5]])

...
import pandas as pd
from supertriplets.dataset import StaticTripletsDataset, OnlineTripletsDataset
from supertriplets.eval import TripletEmbeddingsEvaluator, HardTripletsMiner
...
def test_tinymmimdb_convergence(tinymmimdb_train, tinymmimdb_dev, tinymmimdb_test):
    device = 'cuda:0'
    train_examples = [TextImageSample(text=text, image_path=image_path, label=label) for text, image_path, label in zip(tinymmimdb_train['text'], tinymmimdb_train['image_path'], tinymmimdb_train['label'])]
    dev_examples = [TextImageSample(text=text, image_path=image_path, label=label) for text, image_path, label in zip(tinymmimdb_dev['text'], tinymmimdb_dev['image_path'], tinymmimdb_dev['label'])]
    test_examples = [TextImageSample(text=text, image_path=image_path, label=label) for text, image_path, label in zip(tinymmimdb_test['text'], tinymmimdb_test['image_path'], tinymmimdb_test['label'])]
    ...
    trainset = OnlineTripletsDataset(examples=train_examples, in_batch_num_samples_per_label=2, batch_size=32, sample_loading_func=model.load_input_example)
    devset = StaticTripletsDataset(anchor_examples=dev_anchor_examples, positive_examples=dev_positive_examples, negative_examples=dev_negative_examples, sample_loading_func=model.load_input_example)
    testset = StaticTripletsDataset(anchor_examples=test_anchor_examples, positive_examples=test_positive_examples, negative_examples=test_negative_examples, sample_loading_func=model.load_input_example)
    ...
    triplet_embeddings_evaluator = TripletEmbeddingsEvaluator(calculate_by_cosine = True, calculate_by_manhattan= True, calculate_by_euclidean = True)
    ...
    dev_start_accuracy = triplet_embeddings_evaluator.evaluate(embeddings_anchors=dev_triplet_embeddings['anchors'], embeddings_positives=dev_triplet_embeddings['positives'], embeddings_negatives=dev_triplet_embeddings['negatives'])
    test_start_accuracy = triplet_embeddings_evaluator.evaluate(embeddings_anchors=test_triplet_embeddings['anchors'], embeddings_positives=test_triplet_embeddings['positives'], embeddings_negatives=test_triplet_embeddings['negatives'])
    ...
    dev_triplet_embeddings = get_triplet_embeddings(dataloader=devloader, model=model, device=device)
    test_triplet_embeddings = get_triplet_embeddings(dataloader=testloader, model=model, device=device)
    dev_final_accuracy = triplet_embeddings_evaluator.evaluate(embeddings_anchors=dev_triplet_embeddings['anchors'], embeddings_positives=dev_triplet_embeddings['positives'], embeddings_negatives=dev_triplet_embeddings['negatives'])
    test_final_accuracy = triplet_embeddings_evaluator.evaluate(embeddings_anchors=test_triplet_embeddings['anchors'], embeddings_positives=test_triplet_embeddings['positives'], embeddings_negatives=test_triplet_embeddings['negatives'])
